...
def get_pep_internal_status(obj: PEPObject, session: CachedSession):
    response = get_response(session, obj.internal_link)
    response.encoding = DEFAULT_RESPONSE_ENCODING
    soup = BeautifulSoup(response.text, features=DEFAULT_PARSE_MODE)
    article = find_tag(soup, 'article')
    pattern = re.compile(RegexpTemplates.RFC_TEMPLATE)
...
def pep(session):
    ...
    for obj in tqdm(pep_objects, desc='Обработка данных'):
        internal_status = get_pep_internal_status(obj, session)
        if internal_status not in EXPECTED_STATUS[obj.abbr[1:]]:
            logging.error(
                'Несовпадающие статусы:\n'
                f'{obj.internal_link}\n'
                f'Статус в карточке: {internal_status}\n'
                'Ожидаемые статусы:\n'
                f'{EXPECTED_STATUS[obj.abbr[1:]]}'
            )
        if internal_status in PEP_STATUS_COUNTER.keys():
            PEP_STATUS_COUNTER[internal_status] += 1
        else:
            PEP_STATUS_COUNTER['Other'] += 1

    PEP_STATUS_COUNTER['Total'] = len(pep_objects)
    return [(key, value) for key, value in PEP_STATUS_COUNTER.items()]
...
def latest_versions(session):
    ...
    pattern = RegexpTemplates.PYTHON_VERSION_TEMPLATE
...
def download(session):
    ...
    pdf_tag = table.find('a', {'href': re.compile(
        RegexpTemplates.DOCS_PDF_DOWNLOAD_TEMPLATE
    )})
